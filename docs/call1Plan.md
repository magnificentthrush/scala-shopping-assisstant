...
2. Update `docs/API_CONTRACT.md` Messages section — document the temporary `200 { safe, sessionId, message }` pass body and state that the full assistant shape arrives with Call #2. Keep `422 REJECTED` as-is.
3. Extend `assistant/config/AppConfig.scala` with required `gemmaApiKey` from `GEMMA_API_KEY` (fail fast if unset, same style as `JWT_SECRET`). Confirm `.env.example` already documents `GEMMA_API_KEY`.
4. `assistant/domain/` — `SendMessageRequest`, temporary pass response case class (e.g. `ValidationPassResponse`), with upickle `ReadWriter`s. Reuse `ErrorBody` for errors.
5. `assistant/services/RegexPreFilter.scala` + `RegexPreFilterSpec` — narrow phrase denylist; ≥3 true positives and ≥3 false positives per `ARCHITECTURE.md` §6.
