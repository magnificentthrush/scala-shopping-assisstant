...
6. `assistant/services/MessageValidationService.scala` — orchestrates blank check → regex → `PromptValidator.validate`; returns `Either[ValidationFailure, …]`; never throws. Routes only map status + JSON. — **Done**: `MessageValidationService(client: LLMClient).validate(message, sessionId)` runs blank → `400`, `RegexPreFilter.isBlocked` → `422 REJECTED`, `PromptValidator.validate` fail-closed → `422 REJECTED`, else temporary `ValidationPassResponse`; a `Try` guard makes it never throw. `ValidationFailure(status, error, code)` lives in `assistant/domain/MessageValidation.scala` (internal control-flow type, not serialized directly). `MessageValidationServiceSpec` (fake `LLMClient`, counts calls) verifies all four paths incl. 0-LLM-call on blank/regex — 5/5 green.
7. `assistant/http/MessageRoutes.scala` — `@authed(jwt)` + `POST /api/sessions/:sessionId/messages` + `OPTIONS` preflight; parse body; call service; map to status/JSON (same style as `AuthRoutes`). — **Done**: `MessageRoutes(jwt, validation)` mirrors `AuthRoutes` — `@cask.route(..., methods = Seq("options"))` preflight + `@authed(jwt)` `@cask.post("/api/sessions/:sessionId/messages")`; `parseBody[SendMessageRequest]` → `400 "Invalid JSON body"`; service `Either` → `200 writeJs(ValidationPassResponse)` or `errorJson` (status/error/code → `ErrorBody` shape, no `"code": null`); `userId` injected but unused this pass (ownership later). `sbt compile` clean.
