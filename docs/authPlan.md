...
18. Frontend: `frontend/src/pages/Signup/Signup.tsx` — after signup, show "check your email"; keep a Phase-1-only "Verify now" link when `pendingVerificationToken` was stored (no-Resend fallback). With Resend configured, the real inbox link is the primary path. **Done** — the "Check your email" screen already existed; after a successful `register`, `Signup` now checks `getPendingVerificationToken()` and, when a token was stored (Phase 1, no Resend), renders a primary **Verify now** link to `/verify-email` plus an explanatory note ("No email service is configured, so we stored your link for this session instead"). With Resend configured no token is stored, so the primary action stays "Back to login" and the inbox link is the path. `tsc -b` and `vite build` pass; `oxlint` clean for the changed file.
19. Frontend: `frontend/src/pages/Login/Login.tsx` — show a distinct "please verify your email" message when `EMAIL_NOT_VERIFIED` comes back, instead of the generic error text. **Done** — `Login` catches `ApiError.code === "EMAIL_NOT_VERIFIED"` and renders a distinct "Please verify your email before logging in." alert with a **Verify now** link to `/verify-email` when a Phase-1 `pendingVerificationToken` is stored, or "Check your inbox for the verification link we sent you." otherwise. `tsc -b` and `vite build` pass; `oxlint` clean for the changed file.
20. End-to-end manual test through the actual UI: sign up → open verification email from `@scalainterns.dev` (or Phase 1 "Verify now") → log in → land on `/`. **Done** — exercised Phase 1 through the real UI (`http://localhost:5173`): signup → **Verify now** → login → landed on `/` as the signed-in user. Along the way fixed two blockers that broke the browser path: (1) `VerifyEmail` StrictMode remount discarded the single-use verify response (module-level in-flight promise cache); (2) `apiFetch` always sent `Content-Type: application/json` on GET, forcing a CORS preflight to `/api/auth/verify-email?token=…` that Cask rejected (`Unknown argument: "token"` on `verifyEmailOptions`) — fixed by only setting `Content-Type` when a body is present, and by accepting `token` on the OPTIONS handler.
