...
export default function ChatWidget({ conversationId, sessionId, onFirstMessageSent }: ChatWidgetProps) {
  ...
  async function sendAndAppend(content: string, wasFirstMessage: boolean) {
    try {
  ...
  async function handleSend(file: File | null) {
    ...
    await sendAndAppend(optimisticUserMessage.content, wasFirstMessage);
  }
  ...
  // Editing removes everything after that message and re-sends it as a fresh turn
  async function saveEdit(msg: ChatMessage) {
    ...
    await sendAndAppend(updatedMessage.content, truncatedHistory.length === 0);
  }
                    ...
                    {editingId === msg.id ? (
                      <div className="message__edit">
                        ...
                        <div className="message__edit-actions">
                          ...
                          <button type="button" className="button button--primary" onClick={() => saveEdit(msg)}>
                            Save
                          </button>
                        </div>
                      </div>
